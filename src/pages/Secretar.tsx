...
const secretarData = {
  name: "Nițu Alin-Cosmin", 
  role: "Secretarul General al Comunei Almăj",
  shortRole: "Garantul Legalității Actelor",
  locality: "Comuna Almăj, jud. Dolj",
  email: "primariaalmaj@gmail.com",
  phone: "0251 449 234",
  audienceSchedule: "Luni - Joi: 10:00 - 13:00",
  location: "Primăria Almăj – Biroul Secretarului",
  responseTime: "Răspuns estimat: 48-72h",
  messageShort:
    "Secretarul general asigură respectarea principiului legalității în adoptarea actelor administrative și oferă suport tehnic și juridic consiliului local.",
  messageLong:
    "Secretarul general al comunei este funcționar public de conducere, având ca misiune principală garantarea respectării principiului legalității în adoptarea actelor administrative. Oferim suport tehnic și juridic consiliului local și primarului, protejând în același timp dreptul cetățenilor de a fi informați corect și transparent cu privire la deciziile autorităților locale.",
  transparencyLinks: [
    { label: "Registru Proiecte Hotărâri", href: "/transparenta" },
    { label: "Dispoziții Primar", href: "/organizare" },
  ...
  duties: [
    ...
    "Pregătește lucrările supuse dezbaterii consiliului local și comisiilor de specialitate.",
  ...
  faq: [
    { question: "Cum pot obține o copie după o Hotărâre de Consiliu?", answer: "Puteți depune o cerere scrisă la registratura primăriei sau online, iar copia va fi eliberată conform termenilor legali." },
    { question: "Cum mă programez în audiență?", answer: "Completați formularul de pe această pagină. Veți fi contactat pe email/telefon pentru confirmare." },
    { question: "Unde pot consulta dispozițiile primarului?", answer: "Acestea sunt publicate pe site în secțiunea Transparență și pot fi consultate fizic la sediul instituției." },
  ] as FAQItem[],
};
...
const Secretar = () => {
  ...
  return (
    <PageLayout breadcrumbs={[{ label: "Acasă", href: "/" }, { label: "Administrație", href: "/primaria" }, { label: "Secretar General" }]}>      
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 lg:items-stretch gap-6 lg:gap-8 xl:gap-10">
          <div className="order-1 flex w-full flex-col items-center space-y-5 text-center lg:items-start lg:text-left lg:pr-5 xl:pr-7">
            <div className="fade-in-left gsap-optimize inline-flex">
              <span className="text-[10px] sm:text-xs font-bold uppercase tracking-[0.2em] text-blue-600 bg-blue-50/80 px-3 py-1.5 rounded-md">
                Primăria Almăj
              </span>
            </div>
            ...
            <div className="fade-in-left gsap-optimize flex w-full flex-col gap-3 pt-2 text-sm font-medium text-slate-700 sm:text-base">
              <span className="flex items-center justify-center gap-3 lg:justify-start">
                <MapPin className="w-5 h-5 text-blue-500" /> {secretarData.locality}
              </span>
              <span className="flex items-center justify-center gap-3 lg:justify-start">
                <Phone className="w-5 h-5 text-blue-500" /> 
                <a href={`tel:${secretarData.phone.replace(/\s/g, '')}`} className="hover:text-blue-600 transition-colors">{secretarData.phone}</a>
              </span>
              <span className="flex items-center justify-center gap-3 lg:justify-start">
                <Mail className="w-5 h-5 text-blue-500" /> 
                <a href={`mailto:${secretarData.email}`} className="hover:text-blue-600 transition-colors break-all">{secretarData.email}</a>
              </span>
            </div>
          ...
          <div className="order-3 lg:order-2 fade-in-right gsap-optimize flex w-full flex-col border-t-4 border-blue-100 pt-5 lg:border-t-0 lg:border-l-4 lg:pl-6 xl:pl-8 lg:py-1 lg:justify-between">
            <div>
              <div className="flex items-center gap-3 mb-4 lg:mb-6">
                <Gavel className="w-6 h-6 sm:w-8 sm:h-8 text-blue-600" />
              ...
              >
                {secretarData.messageLong}
              </p>
              <p
                className="text-base font-medium leading-relaxed text-slate-800 lg:hidden sm:text-lg"
                style={{ textIndent: "1.5rem" }}
              >
                {expandedMessage ? secretarData.messageLong : secretarData.messageShort}
              </p>
              <button 
          ...
          <section id="audienta" className="fade-up-scroll gsap-optimize scroll-mt-24 lg:col-start-1" aria-labelledby="audienta-title">
            <div className="space-y-4 sm:space-y-5">
                <div className="space-y-3">
                  ...
                  <p className="text-slate-700 text-sm sm:text-base font-medium leading-relaxed">Completați formularul pentru a programa o discuție cu Secretarul General.</p>
                </div>
                ...
                <form ref={formRef} onSubmit={handleSubmit} className="space-y-4 sm:space-y-5 border-t border-slate-200 pt-5 sm:pt-6">
                    ...
                    <div className="space-y-2">
                      ...
                      <Textarea name="message" rows={4} required className="bg-slate-50 border-slate-200 focus:bg-white focus:ring-2 focus:ring-blue-500/20 transition-all rounded-xl resize-none text-base p-4" placeholder="Descrieți pe scurt subiectul discuției (ex: eliberare acte, consultații)..." />
                    </div>
          ...
          <section className="fade-up-scroll gsap-optimize lg:col-start-2" aria-labelledby="duties-title">
            <div className="space-y-4">
                <div className="space-y-3">
                  ...
                  <h2 id="duties-title" className="text-2xl sm:text-3xl font-black text-slate-900 tracking-tight">Atribuții Legale</h2>
                  <p className="text-slate-700 text-sm sm:text-base font-medium">Codul Administrativ definește ariile stricte de expertiză.</p>
                </div>

                <Accordion type="single" collapsible className="border-t border-slate-200">
                  {secretarData.duties.map((duty, idx) => (
            ...
            <div className="space-y-5">
                <div className="space-y-3">
                  ...
                  <p className="text-sm sm:text-base text-slate-700 font-medium leading-relaxed">
                    Detalii despre programări și eliberarea documentelor oficiale.
                  </p>
                </div>

                <Accordion type="single" collapsible className="border-t border-slate-200">
                  {secretarData.faq.map((item, idx) => (
                    <AccordionItem
          ...
          <section className="fade-up-scroll gsap-optimize lg:col-start-2" aria-labelledby="transparency-title">
            <div className="space-y-4">
                <div className="space-y-3">
                  ...
                  <h2 id="transparency-title" className="text-2xl sm:text-3xl font-black text-slate-900 tracking-tight">Transparență Acte</h2>
                  <p className="text-slate-700 text-sm sm:text-base font-medium">Registre publice și hotărâri oficiale curente.</p>
                </div>

                <div className="border-t border-slate-200">
                  {secretarData.transparencyLinks.map((link, idx) => (
                    <a
